...
bool isFloatValue(const string &inputStr) {
    ...
    return tempStr.find('.') != string::npos || tempStr.find('e') != string::npos || tempStr.find('E') != string::npos;
}
...
string generateAssemblyCode(const string &TAC) {
    ...
    if (regex_search(expression, expressionMatches, expressionPattern)) {
        ...
        bool isResultFloat = isFloatValue(operand1) || isFloatValue(operand2) || isFloatVariableName(operand1) ||
            isFloatVariableName(operand2);
        string assemblyCode;
        ...
        if (operatorType == 1) {
            if (isNumericValue(operand1)) {_
                if (isFloatValue(operand1) || isFloatValue(operand2)) {
                    assemblyCode += "ADDF #" + operand1 + ", R1\n";
                } else {
                    assemblyCode += "ADD #" + operand1 + ", R1\n";
                }
            } else {
            ...
            if (isNumericValue(operand1)) {
                if (isFloatValue(operand1) || isFloatValue(operand2)) {
                    assemblyCode += "SUBF #" + operand1 + ", R1\n";
                } else {
                    assemblyCode += "SUB #" + operand1 + ", R1\n";
                }
            } else {
            ...
            if (isNumericValue(operand1)) {
                if (isFloatValue(operand1) || isFloatValue(operand2)) {
                    assemblyCode += "MULF #" + operand1 + ", R1\n";
                } else {
                    assemblyCode += "MUL #" + operand1 + ", R1\n";
                }
            } else {
            ...
            if (isNumericValue(operand1)) {
                if (isFloatValue(operand1) || isFloatValue(operand2)) {
                    assemblyCode += "DIVF #" + operand1 + ", R1\n";
                } else {
                    assemblyCode += "DIV #" + operand1 + ", R1\n";
                }
            } else {
