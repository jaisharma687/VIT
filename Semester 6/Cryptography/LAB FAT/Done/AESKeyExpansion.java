...
public class AESKeyExpansion {
    ...
    private static int subByte(int byteValue) {
        return SBox[(byteValue >> 4) & 0x0F][byteValue & 0x0F];
    }
    ...
    private static void xorWords(int[] result, int[] word1, int[] word2) {
        for (int i = 0; i < 4; i++) {
            result[i] = word1[i] ^ word2[i];
        }
    }

    private static void printWord(String label, int[] word) {
        System.out.print(label + " = ");
        for (int i = 0; i < 4; i++) {
            System.out.printf("%02X ", word[i]);
        }
        System.out.println();
    }
